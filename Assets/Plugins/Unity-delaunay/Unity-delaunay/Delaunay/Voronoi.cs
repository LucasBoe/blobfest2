...
namespace Delaunay
{
    public sealed class Voronoi : Utils.IDisposable
    {
        ...
        public List<Site> Sites => _sites.Sites;
        ...
        public List<Vector2> Relax(List<Vector2> points)
        {
            ...
            foreach (var point in points)
            {
                ...
                foreach (var edge in _sitesIndexedByLocation[point].Edges)
                {
                    var seg = edge.VoronoiEdge();

                    foreach (var p in new Vector2?[] { seg.p0, seg.p1 })
                    {
                        if (p.HasValue)
                        {
                            float weight = 1f;
                            //float weight = edge.SitesDistance();

                            center += p.Value * weight;
                            totalWeight += weight;
                        }
                    }
                }
